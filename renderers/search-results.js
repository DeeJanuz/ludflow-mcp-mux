...
(function () {
  ...
  // Use shared citation utilities
  var _sharedUtils = window.__companionUtils || {};
  var CITE_TYPE_MAP = _sharedUtils.CITE_TYPE_MAP || {};
  var buildCitationMap = _sharedUtils.buildCitationMap || function () { return {}; };
  ...
  window.__renderers.search_results = function renderSearchResults(container, data, meta, toolArgs, reviewRequired, onDecision) {
    ...
    if (data && data.id && !data.data && !Array.isArray(data)) {
      if (utils && utils.companionFetch) {
        var statusEl = document.createElement('div');
        statusEl.style.cssText = 'padding: 16px; color: var(--text-secondary); text-align: center;';
        statusEl.textContent = 'Loading...';
        container.appendChild(statusEl);
        utils.companionFetch('search_codebase', { id: data.id })
          .then(function (result) {
            container.removeChild(statusEl);
            if (result && result.data) {
              renderSearchResults(container, result.data, meta, toolArgs, reviewRequired, onDecision);
            }
          })
          .catch(function (err) {
